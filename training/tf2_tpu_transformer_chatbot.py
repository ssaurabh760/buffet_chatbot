# -*- coding: utf-8 -*-
"""tf2_tpu_transformer_chatbot.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1yR3mM4H_d9tL4LPbtnubjBFzSAc-M07v
...
file_path = './reminiscences_of_a_stock_operator_qa.csv'
...
for i in range(len(questions)):
  l1 = len(questions[i].split())
  l2 = len(answers[i].split())
  if l1 > MAX_LENGTH:
    MAX_LENGTH = l1
  if l2 > MAX_LENGTH:
    MAX_LENGTH = l2
...
# def prepare_dataset(questions, answers, tokenizer, max_length=40, batch_size=32):
...
#         if i % 5000 == 0:
#             print(f"  Processed {i}/{len(answers)}")
...
#     question_tokens = tf.cast(question_tokens, tf.int32)
#     answer_tokens = tf.cast(answer_tokens, tf.int32)

#     print("Creating dataset...")
#     dataset = tf.data.Dataset.from_tensor_slices((question_tokens, answer_tokens))
#     dataset = dataset.shuffle(buffer_size=min(len(questions), 10000))
#     dataset = dataset.batch(batch_size)
#     dataset = dataset.prefetch(tf.data.AUTOTUNE)

#     print(f"✅ Dataset prepared!")
#     print(f"   Questions: {question_tokens.shape}")
#     print(f"   Answers: {answer_tokens.shape}")

#     return dataset


# # Use it
# dataset = prepare_dataset(questions, answers, tokenizer, MAX_LENGTH, BATCH_SIZE)
# print("\n✅ Ready for training!")
...
# Create dataset with simple format
import tensorflow as tf

questions_tensor = tf.cast(questions, tf.int32)
answers_tensor = tf.cast(answers, tf.int32)

# Simple tuple format: (inputs, targets)
dataset = tf.data.Dataset.from_tensor_slices((
    (questions_tensor, answers_tensor[:, :-1]),  # inputs
    answers_tensor[:, 1:]                         # targets
))
...
def encoder(vocab_size, num_layers, units, d_model, num_heads, dropout, name="encoder"):
    ...
    # ✅ FIXED: Use Lambda instead of custom PositionalEncoding
    def add_pos_enc(x):
        seq_length = tf.shape(x)[1]
        position = tf.range(seq_length, dtype=tf.float32)[:, tf.newaxis]
        dim_indices = tf.range(d_model, dtype=tf.float32)[tf.newaxis, :]
        angles = position / tf.pow(10000.0, (2 * (dim_indices // 2)) / tf.cast(d_model, tf.float32))
        angles = tf.where(tf.cast(dim_indices, tf.int32) % 2 == 0, tf.sin(angles), tf.cos(angles))
        return x + angles[tf.newaxis, :, :]

    embeddings = tf.keras.layers.Lambda(add_pos_enc)(embeddings)
...
def decoder(vocab_size, num_layers, units, d_model, num_heads, dropout, name="decoder"):
    ...
    # ✅ FIX 1: Direct computation (not Lambda)
    embeddings *= tf.math.sqrt(tf.cast(d_model, tf.float32))

    # ✅ FIX 2: Positional encoding with Lambda to avoid cycles
    def add_pos_enc(x):
        seq_length = tf.shape(x)[1]
        position = tf.range(seq_length, dtype=tf.float32)[:, tf.newaxis]
        dim_indices = tf.range(d_model, dtype=tf.float32)[tf.newaxis, :]
...
# Download tokenizer
files.download('tokenizer_vocab')
